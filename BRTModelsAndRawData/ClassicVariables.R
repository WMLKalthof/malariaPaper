...
data <-  read.csv("statsCombinedDocuments/allVariablesCombinedClassic.csv")
...
RCModel1 = data.frame(matrix(ncol=0,nrow=8))
Rx1 <- c("NDVI_b50", "NDVI_b10000", "tempAv_b1000", "tempAv_b100000", 
         "elevation_b100000", "elevation_b50", "travelTime_b100000", "travelTime_b1000")
RCModel1 <- RCModel1 %>% cbind(RCModel1, Rx1) %>% rename("var" = "Rx1")

RCModel2 = data.frame(matrix(ncol=0,nrow=18))
Rx2 <- c("NDVI_b50", "NDVI_b10000", "precMax_b50", "precMax_b10000", "precMin_b50", "precMin_b100000", "precMaxAmplitude_b50", "precMaxAmplitude_b10000", "tempAv_b1000", "tempAv_b100000", 
         "elevation_b100000", "elevation_b50", "travelTime_b100000", "travelTime_b1000", "precAv_b100000", "precAv_b50", "waterNDWI_b100000p50", "distance30_b50p50")
...
for ( i in 1:50){
  ...
  data1 <- select(training, c(pf_pr, NDVI_b50, NDVI_b10000, tempAv_b1000, tempAv_b100000, elevation_b100000, elevation_b50, travelTime_b100000, travelTime_b1000))
  BRT_malaria1 <- gbm.fixed(data=data1, gbm.x=2:9 ,gbm.y=1, family="gaussian", tree.complexity = 10, learning.rate = 0.005, n.trees = 5000,  bag.fraction = 0.6)
  
  
  #model 2: Confounders + solano-villareal et al variables: NDWI at 100km buffer, distance to seasonal water bodies (50% persistence) -- classic scenario
  data2 <- select(training, c(pf_pr, NDVI_b50, NDVI_b10000, precMax_b50, precMax_b10000, precMin_b50, precMin_b100000, precMaxAmplitude_b50, precMaxAmplitude_b10000,
                              tempAv_b1000, tempAv_b100000, elevation_b100000, elevation_b50, travelTime_b100000, travelTime_b1000, precAv_b100000, precAv_b50,
                              waterNDWI_b100000p50, distance30_b50p50))
  BRT_malaria2 <- gbm.fixed(data=data2, gbm.x=2:19 ,gbm.y=1, family="gaussian", tree.complexity = 10, learning.rate = 0.005, n.trees = 5000,  bag.fraction = 0.6)
  ...
  # Var1 <- partial(BRT_malaria1, pred.var = c("waterNDWI_b100000p50"), train = data1, n.trees = 2000)
  # Var1Y <- cbind(Var1Y, Var1$yhat)
  # 
  # Var2 <- partial(BRT_malaria1, pred.var = c("distanceClassic_b50p70"), train = data1, n.trees = 2000)
  # Var2Y <- cbind(Var2Y, Var2$yhat)
  # 
  # Var3 <- partial(BRT_malaria2, pred.var = c("distanceClassic_b50p10"), train = data2, n.trees = 2000)
  # Var3Y <- cbind(Var3Y, Var3$yhat)
  # 
  # Var4 <- partial(BRT_malaria2, pred.var = c("waterArea_b10000p50"), train = data2, n.trees = 2000)
